...
parser.add_argument('--c', dest='c', type=int, default='2', help='the number of NN considered, 1 by default')
...
for line in open(args.groundtruth_path, 'r'):
	line = line.replace('[', '').replace(']', '') #comment for hog
	parts = line.split(',')
...
for p, d, f in os.walk(args.approx_path):
	...
	out = open(os.path.join(args.out_path, "c%s_k%d"%(args.c, args.k)), 'w')
